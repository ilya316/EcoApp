    ...
    def handle(self, *args, **kwargs):
        ...
        with open(script_directory + '\\Wastes\\management\\commands\\types.json', 'r', encoding='utf8') as data:
            for item in json.loads(data.read()):
                ...
                new_type.save()
        # Containers
        with open(script_directory + '\\Wastes\\management\\commands\\container_sites.json', 'r') as data:
            for item in json.loads(data.read()):
                ...
                try:
                    ...
                    pass
        # Large containers
        with open(script_directory + '\\Wastes\\management\\commands\\large_containers.json', 'r') as data:
            for item in json.loads(data.read()):
                try:
                    ...
                    pass
        # Lamps
        with open(script_directory + '\\Wastes\\management\\commands\\lamps.json', 'r') as data:
            for item in json.loads(data.read()):
                try:
                    _lattitude, _longitude = item['geoData']['coordinates']
                    new_site = WasteSite(
